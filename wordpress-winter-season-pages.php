        ...
        foreach ($groups[$class] as $district => $rows) {
            ...
            foreach ($rows as $row) {
                ...
                $out .= '<article class="lvay-w-school" id="' . esc_attr($cfg['short'] . '-' . sanitize_title($name))
                    ...
                    . '<button type="button" aria-expanded="false"><b>' . esc_html($name) . '</b><span>' . esc_html($record) . '</span></button><div hidden></div></article>';
            }
            $out .= '</div></details>';
        }
        $out .= '</div></details>';
...
function lvay_winter_brackets($cfg, $season) {
    ...
    foreach (array('Division I Non-Select'=>array('I',0),'Division II Non-Select'=>array('II',0),'Division III Non-Select'=>array('III',0),'Division IV Non-Select'=>array('IV',0),'Division I Select'=>array('I',1),'Division II Select'=>array('II',1),'Division III Select'=>array('III',1),'Division IV Select'=>array('IV',1),'Class B'=>array('B',null),'Class C'=>array('C',null)) as $label=>$parts) {
        $endpoint = $parts[1] === null ? 'MainBracket32Print_2.aspx' : 'MainBracket32.aspx';
        $url = 'https://www.lhsaaonline.org/' . $endpoint . '?d=' . rawurlencode($parts[0]) . '&s=' . (int)$cfg['bracket_sport'] . '&y=2026';
...
function lvay_winter_assets() {
    ...
    .lvay-w-search{position:relative;z-index:30;margin-bottom:18px}.lvay-w-search input{display:block;width:100%;height:50px;border:2px solid #078b88;border-radius:4px;padding:10px 15px;font-size:18px}.lvay-w-results{position:absolute;z-index:9999;top:100%;left:0;right:0;background:#fff;border:1px solid #078b88;max-height:420px;overflow:auto;box-shadow:0 10px 24px #0003}.lvay-w-results button{display:block;width:100%;text-align:left;border:0;border-bottom:1px solid #ddd;background:#fff!important;color:#111!important;padding:10px 14px;font:500 24px Teko}.lvay-w-results button:hover,.lvay-w-results button:focus{background:#000!important;color:#fff!important}
    ...
    document.addEventListener("DOMContentLoaded",function(){
      ...
      function openSchool(card){
        ...
        if(!body.dataset.loaded){body.innerHTML="<p>Loading schedule...</p>";fetch("https://lvay-scraper.onrender.com/api/schedules/winter/"+sport+"?season="+season+"&school="+encodeURIComponent(card.dataset.label)).then(function(r){return r.json()}).then(function(data){
          ...
          var h="<table><thead><tr><th>Date</th><th>H/A</th><th>Opponent</th><th>W/L</th><th>Score</th><th>Division</th><th>Power Pts</th></tr></thead><tbody>";
          (s.games||[]).forEach(function(g){var cls="lvay-w-game "+((g.result||"").toLowerCase())+(g.is_district?" district":"");var opp=(g.opp_wins!=null&&g.opp_losses!=null)?" ("+g.opp_wins+"-"+g.opp_losses+(g.opp_ties?"-"+g.opp_ties:"")+")":"";h+="<tr class=\""+cls+"\"><td>"+esc(g.game_date||"")+"</td><td>"+esc(g.home_away||"")+"</td><td>"+opponentLink(g.opponent||"")+esc(opp)+"</td><td>"+esc(g.result||"")+"</td><td>"+esc(g.score||"")+"</td><td>"+esc(g.opp_division||"")+"</td><td>"+(g.total_pts==null?"":Number(g.total_pts).toFixed(2))+"</td></tr>"});body.innerHTML=h+"</tbody></table>";body.dataset.loaded="1";
