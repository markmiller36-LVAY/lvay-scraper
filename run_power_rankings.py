...
def run_power_rankings(season=SEASON, sport=SPORT):
    ...
    for r in rows:
        ...
        db_info = get_school(school, sport, season)
    ...
    for r in rows:
        ...
        opp_in_db = get_school(opponent, sport, season) is not None
    ...
    for r in ratings:
        db_info = get_school(r.name, sport, season)
        ...
        for g in r.breakdown:
            ...
            meta = game_meta.get(game_key, {})
            school_info = get_school(r.name, sport, season)
            opp_info = get_school(
                meta.get("opponent", g["opponent"]), sport, season
            )
