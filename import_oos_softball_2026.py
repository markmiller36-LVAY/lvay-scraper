"""
import_oos_softball_2026.py
============================
Pulls OOS opponent win/loss/tie records from the Google Sheet tab
"Softball OOS Opponents (2026)" and stores them in the oos_opponents
...
Run order:
  ...
  2. THIS script
  3. run_power_rankings.py (sport=softball, season=2026)
"""
import os
import sqlite3
import gspread
...
TAB_NAME    = "Softball OOS Opponents (2026)"
SPORT       = "softball"
SEASON      = "2026"
...
def run():
    print(f"\n--- OOS Softball Import (season={SEASON}) ---")

    try:
        rows = get_sheet_data()
    except Exception as e:
        raise RuntimeError(f"Softball OOS Sheet read failed: {e}") from e
